...
static void drawLine(int x0, int y0, int x1, int y1, int width, Color color, Color* destination)
{
	int dy = y1 - y0;
	int dx = x1 - x0;
	int stepx, stepy;
...
{
	drawLine(x0, y0, x1, y1, PSP_LINE_SIZE, color, getVramDrawBuffer());
}

void drawLineImage(int x0, int y0, int x1, int y1, Color color, Image* image)
{
	drawLine(x0, y0, x1, y1,image->textureWidth, color, image->data);
}
...
{
	fillScreenRect(col,0,0,SCREEN_WIDTH,SCREEN_HEIGHT);
}
...
void drawRect(int x0, int y0, int x1, int y1, Color color)
{
	...
	drawLineScreen(x0,        y0,        x0+temp_x, y0,        color);
	drawLineScreen(x0+temp_x, y0,        x0+temp_x, y0+temp_y, color);
	drawLineScreen(x0+temp_x, y0+temp_y, x0,        y0+temp_y, color);
	drawLineScreen(x0,        y0+temp_y, x0,        y0,        color);
}
